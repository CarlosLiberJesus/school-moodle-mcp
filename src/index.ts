...
async function startServer() { // Renamed main to startServer for clarity
  const app = express();
  ...
  app.post('/mcp', async (req, res) => {
    ...
    try {
      ...
      res.on('close', () => {
        ...
        if (transport) { // Check if transport was initialized
          transport.close();
        }
      ...
      if (transport! && !res.headersSent) { // Check if transport was initialized
         transport.close();
      }
      if (!res.headersSent) {
        res.status(500).json({
  ...
  app.get('/mcp', (req, res) => { // Made synchronous as it doesn't await
    console.log('Received GET /mcp request; Method Not Allowed.');
    res.status(405).json({ // Use .status().json() for consistency
      jsonrpc: "2.0",
      error: {
        code: -32000,
  ...
  app.delete('/mcp', (req, res) => { // Made synchronous
