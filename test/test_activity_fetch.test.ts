...
try {
    ...
    const detailsById = await mcpServer.callToolForTests('get_activity_details', { activity_id: 150 });
    console.log('Result:', JSON.stringify(detailsById, null, 2));

    console.log('\nTest 1.2: get_activity_details by course_name + activity_name');
    const detailsByNames = await mcpServer.callToolForTests('get_activity_details', {
      course_name: "Aplicações Informáticas B 12º Ano Cópia 1",
    ...
    const courseContents = await mcpServer.callToolForTests('get_course_contents', { course_id: 6 });
    ...
    const pageContent = await mcpServer.callToolForTests('get_page_module_content', {
    ...
    const resourceContent = await mcpServer.callToolForTests('get_resource_file_content', {
    ...
    const fetchedContentById = await mcpServer.callToolForTests('fetch_activity_content', { activity_id: 150 });
    ...
    const fetchedContentByNames = await mcpServer.callToolForTests('fetch_activity_content', {
      course_name: "Aplicações Informáticas B 12º Ano Cópia 1",
      activity_name: "Componentes Fundamentais de um PC"
    });
