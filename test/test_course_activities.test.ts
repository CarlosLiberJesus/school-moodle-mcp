...
import type { MoodleApiClient as MoodleApiClientType } from '../moodle/moodle_api_client';
import type { McpError, ErrorCode } from '@modelcontextprotocol/sdk/types'; // Adjust path if needed

// Define a Jest mock function for getCourseContents
const mockGetCourseContents = jest.fn();
...
jest.unstable_mockModule('../moodle/moodle_api_client', () => ({
...
beforeAll(async () => {
  ...
  const MoodleMCPModule = await import('../src/mcp_server');
...
describe('MoodleMCP - get_course_activities Tool (with unstable_mockModule)', () => {
  test('Test Case 1: Successful retrieval of activities', async () => {
    ...
    const mockCourseContentsData = [
      {
        id: 1, name: "Section 1", modules: [
          { id: 101, name: "Activity 1", url: "http://moodle.example.com/mod/assign/view.php?id=101", timemodified: 1678886400, modname: "assign", course: 1, instance: 1, description: "", visible: 1, uservisible: true, visibleoncoursepage: 1, modicon: "", modplural: "Assignments", availabilityinfo: null, indent: 0 },
          { id: 102, name: "Activity 2", url: "http://moodle.example.com/mod/quiz/view.php?id=102", timemodified: 1678972800, modname: "quiz", course: 1, instance: 2, description: "", visible: 1, uservisible: true, visibleoncoursepage: 1, modicon: "", modplural: "Quizzes", availabilityinfo: null, indent: 0 }
        ]
      }
    ];
    ...
    mockGetCourseContents.mockImplementation(async (calledCourseId) => {
      if (calledCourseId === course_id) {
        ...
        console.log('Raw module data from mock (to be processed by tool):', mockCourseContentsData[0].modules[0]);
    ...
    expect(result).toEqual([
      { id: 101, name: "Activity 1", url: "http://moodle.example.com/mod/assign/view.php?id=101", timemodified: 1678886400 },
      { id: 102, name: "Activity 2", url: "http://moodle.example.com/mod/quiz/view.php?id=102", timemodified: 1678972800 }
    ]);
  ...
  test('Test Case 2: Course with no activities/modules', async () => {
    const course_id = 2;
    const test_token = 'test_token';
    const mockDataNoModules = [
      { id: 1, name: "Section 1", modules: [] },
      { id: 2, name: "Section 2", modules: [] }
    ];
  ...
  test('Test Case 3: Course with no sections', async () => {
    const course_id = 3;
    const test_token = 'test_token';
    mockGetCourseContents.mockResolvedValue([]);
  ...
  test('Test Case 4: Invalid course_id (Moodle API error simulation)', async () => {
    ...
    const { McpError, ErrorCode } = await import('@modelcontextprotocol/sdk/types'); // Dynamic import for clarity or use top-level

    mockGetCourseContents.mockRejectedValue(new McpError(ErrorCode.NotFound, "Simulated Moodle API error: Course not found"));
    ...
    try {
      ...
      expect(e.code).toEqual(ErrorCode.NotFound);
      expect(e.message).toContain("Simulated Moodle API error: Course not found");
    }
    expect(mockGetCourseContents).toHaveBeenCalledWith(course_id);
  });
  ...
  test('Test Case 5: Modules with missing optional fields (url, timemodified)', async () => {
    ...
    const mockDataMissingFields = [
      {
        id: 1, name: "Section 1", modules: [
          { id: 201, name: "Activity With Null URL", url: null, timemodified: 1678886400, modname: "assign", course: 1, instance: 1 },
          { id: 202, name: "Activity With Undefined Fields", modname: "quiz", course: 1, instance: 2 } // url and timemodified are undefined
        ]
      }
    ];
    ...
    expect(result).toEqual([
      { id: 201, name: "Activity With Null URL", url: null, timemodified: 1678886400 },
      { id: 202, name: "Activity With Undefined Fields", url: null, timemodified: 0 }
    ]);
    expect(mockGetCourseContents).toHaveBeenCalledWith(course_id);
  });
});
