...
async function testActivityFetch() {
  try {
    ...
    const activityDetails = await mcpServer.callTool('get_activity_details', { activity_id: 150 });
    ...
    const content = await mcpServer.callTool('fetch_activity_content', { activity_id: 150 });
